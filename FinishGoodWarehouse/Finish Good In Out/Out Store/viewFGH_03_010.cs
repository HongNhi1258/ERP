/*
  Author      : Duong Minh
  Date        : 08/07/2014
  Description : Issuing For Container(STC)
*/
using System;
...
{
  public partial class viewFGH_03_010 : MainUserControl
  {
    #region Field
    ...
    {
      string commandText = " SELECT Pid, ContainerNo ";
      ...
      ultContainer.DisplayLayout.Bands[0].Columns["ContainerNo"].Width = 500;
    ...
    private void LoadData()
    {
      ...
      if (this.outStorePid == long.MinValue)
      {
        ...
        commandText += "SELECT dbo.FWHFGetNewOutStoreCode('09STC')";
      ...
      else
      {
        ...
        DataSet ds = DataBaseAccess.SearchStoreProcedure("spWHFOutStoreIssuingToProductionSTC_Select", arrInputParam);
        if (ds != null)
        {
          if (ds.Tables[0].Rows.Count > 0)
          {
            ...
            this.txtIssuingCode.Text = this.issCode;
            this.ultContainer.Value = DBConvert.ParseLong(ds.Tables[0].Rows[0]["ContainerPid"].ToString());
          }
    ...
    private void SetStatusControl()
    {
      if (this.status == 1)
      {
        ...
        this.ultContainer.Enabled = false;
      ...
      else
      {
        ...
        this.ultContainer.Enabled = true;
        if (this.outStorePid == long.MinValue)
        {
          this.chkConfirm.Enabled = false;
    ...
    private void SendEmailWhenFinishContainer()
    {
      ...
      inputParent[0] = new DBParameter("@ContainerPid", DbType.Int64, DBConvert.ParseLong(this.ultContainer.Value.ToString()));
      ...
      if (dtLoadingList != null && dtLoadingList.Rows.Count > 0)
      {
        for (int i = 0; i < dtLoadingList.Rows.Count; i++)
        {
          ...
          input[0] = new DBParameter("@ContainerPid", DbType.Int64, DBConvert.ParseLong(this.ultContainer.Value.ToString()));
    ...
    private void ultSummary_InitializeLayout(object sender, InitializeLayoutEventArgs e)
    {
      ...
      e.Layout.Bands[0].Columns["IssueQty"].Header.Caption = "Iss Qty";
      e.Layout.Bands[0].Columns["PLNLoadQty"].Header.Caption = "Loading Qty";
    ...
    {
      if (this.ultContainer.Value == null || this.ultContainer.Value.ToString().Length == 0)
      {
        string message = "Please fill Container";
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
      ...
      if (this.chkConfirm.Checked)
      {
        for (int i = 0; i < ultSummary.Rows.Count; i++)
        {
          ...
          if (DBConvert.ParseInt(row.Cells["Flag"].Value.ToString()) == 1)
          {
            string message = "Can not issue over qty Loading Qty";
            WindowUtinity.ShowMessageErrorFromText(message);
            return;
          }
        }
      ...
      if (this.outStorePid == long.MinValue)
      {
        ...
        arrInputParam[4] = new DBParameter("@ContainerPid", DbType.Int64, DBConvert.ParseLong(this.ultContainer.Value.ToString()));
      ...
      // Deduct COntainer Auto
      if (this.chkConfirm.Checked && this.ultOutStoreDetail.Rows.Count > 0)
      {
        DBParameter[] arrInputParamDeduct = new DBParameter[2];
        arrInputParamDeduct[0] = new DBParameter("@IssPid", DbType.Int64, this.outStorePid);
        arrInputParamDeduct[1] = new DBParameter("@ContainerPid", DbType.Int64, DBConvert.ParseLong(this.ultContainer.Value.ToString()));

        DBParameter[] arrOutParamDeduct = new DBParameter[1];
        arrOutParamDeduct[0] = new DBParameter("@Result", DbType.Int64, 0);

        DataBaseAccess.ExecuteStoreProcedure("spPLNDeductContainerAuto_Exec", 6000, arrInputParamDeduct, arrOutParamDeduct);
    ...
    {
      int row = ultOutStoreDetail.Rows.Count;
      if (row == 0)
      {
        DBParameter[] arrInputParam = new DBParameter[1];
        arrInputParam[0] = new DBParameter("@IssPid", DbType.Int64, this.outStorePid);

        DataBaseAccess.ExecuteStoreProcedure("spWHFOutStore_UpdateFinish", arrInputParam);
        this.LoadData();
      }
    }
    #endregion Event
