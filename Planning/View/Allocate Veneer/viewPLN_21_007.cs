...
{
  public partial class viewPLN_21_007 : MainUserControl
  {
    #region Field
    #endregion Field
    ...
    /// <param name="e"></param>
    private void viewPLN_21_007_Load(object sender, EventArgs e)
    {
      this.LoadWorkOrder();
    ...
    private void LoadData()
    {
      ...
      {
        input[1] = new DBParameter("@Carcass", DbType.String, ultCBCarcass.Value.ToString());
      }
      ...
      DataTable dt = DataBaseAccess.SearchStoreProcedureDataTable("spPLNVeneerGroupStatus", input);
      if (dt == null)
      {
        return;
      }
    ...
    private bool SaveData()
    {
      for (int i = 0; i < ultData.Rows.Count; i++)
      {
            ...
            && DBConvert.ParseInt(row.Cells["Close"].Value.ToString()) == 1)
        {
          ...
          input[1] = new DBParameter("@CarcassCode", DbType.String, row.Cells["Carcass"].Value.ToString());
          ...
          // Execute Store
          DataBaseAccess.ExecuteStoreProcedure("spPLNVeneerCloseWO_Insert", input, output);
