...
{
  public partial class viewPLN_05_013 : MainUserControl
  {
    #region Field
    private int year = 0;
    ...
    private bool SaveData()
    {
      ...
      for (int i = 0; i < dtQuota.Rows.Count; i++)
      {
        ...
        {
          isFinish = this.UpdateRow(rowQuota, 3);
        }
      }
      // 2: Save Skill Worker
      DataTable dtData = (DataTable)ultSkillWorker.DataSource;
      for (int j = 0; j < dtData.Rows.Count; j++)
      {
        DataRow rowData = dtData.Rows[j];
        if (rowData.RowState != DataRowState.Deleted && rowData.RowState == DataRowState.Modified)
        {
          isFinish = this.UpdateRow(rowData, 4);
        }
      }
    ...
    private bool UpdateRow(DataRow rowQuota, int Type)
    {
      ...
      string strData = "";
      for (int c = 1; c < rowQuota.Table.Columns.Count; c++)
      {
    ...
    private void LoadDataGrid()
    {
      ...
      DataSet dsTarget = Shared.DataBaseUtility.DataBaseAccess.SearchStoreProcedure("spPLNDivisionTargetByHRD_Select", inputParam);
      this.ultraQuota.DataSource = dsTarget.Tables[0];
      this.ultSkillWorker.DataSource = dsTarget.Tables[1];
    ...
    private void ultSkillWorker_InitializeLayout(object sender, InitializeLayoutEventArgs e)
    {
      e.Layout.Reset();
      Shared.Utility.ControlUtility.SetPropertiesUltraGrid(ultSkillWorker);
    ...
    private void ultSkillWorker_BeforeCellUpdate(object sender, BeforeCellUpdateEventArgs e)
    {
      string text = e.Cell.Text;
      if (text.Trim().Length == 0)
      {
