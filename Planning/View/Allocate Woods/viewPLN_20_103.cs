...
{
  public partial class viewPLN_20_103 : DaiCo.Shared.MainUserControl
  {
    #region Init
    public long departmentAllocatePid = long.MinValue;

    public viewPLN_20_103()
    {
      InitializeComponent();
    }
    ...
    private void viewPLN_07_103_Load(object sender, EventArgs e)
    {
      if (departmentAllocatePid != long.MinValue)
      {
        ...
        commandText += " SELECT Department, SU.[Group], SU.Category, CAT.Name, BOH, Month, Year";
        commandText += " FROM TblPLNWoodsAllocateDeptSummary SU ";
        commandText += " 	INNER JOIN TblGNRMaterialCategory CAT ON SU.[Group] = CAT.[Group] ";
        commandText += " 											AND SU.Category = CAT.Category ";
        commandText += " WHERE SU.Pid = " + this.departmentAllocatePid;
        DataTable dt = DataBaseAccess.SearchCommandTextDataTable(commandText);
        if (dt.Rows.Count > 0)
        {
          this.txtDepartment.Text = dt.Rows[0]["Department"].ToString();
          this.txtCode.Text = dt.Rows[0]["Group"].ToString() + "-"
          ...
          commandText += " FROM TblPLNWoodsAllocateForDepartment AD";
          ...
          commandText += " AND AD.[Group] = '" + dt.Rows[0]["Group"].ToString() + "' ";
          commandText += " AND AD.Category = '" + dt.Rows[0]["Category"].ToString() + "' ";
