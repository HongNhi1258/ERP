...
namespace DaiCo.Purchasing
{
  public partial class viewPUR_21_002 : MainUserControl
  {
    ...
    private bool flagHideWOCarcass = false;
    ...
    private void viewPUR_21_002_Load(object sender, EventArgs e)
    {
      ...
      if (this.btnHideWO.Visible == true)
      {
        this.flagHideWOCarcass = true;
        this.btnHideWO.Visible = false;
      }
    ...
    private void LoadCarcass(long wo)
    {
      ...
      ultddCarcass.DataSource = dtCarcass;
    ...
    private UltraDropDown LoadDDRevision(UltraDropDown ultdd, string ItemCode)
    {
      ...
      DataTable dtSource = DataBaseAccess.SearchCommandTextDataTable(commandText);
      Shared.Utility.ControlUtility.LoadUltraDropDown(ultdd, dtSource, "Revision", "Revision");
      return ultdd;
    }
    ...
    private void LoadItem(long wo)
    {
      ...
      ultddItem.DataSource = dtItemCode;
      ultddItem.DisplayMember = "ItemCode";
      ultddItem.ValueMember = "ItemCode";
      ultddItem.DisplayLayout.Bands[0].ColHeadersVisible = false;
    ...
    private void LoadMaterial()
    {
      ...
      ultddMaterial.DisplayLayout.Bands[0].ColHeadersVisible = true;
    ...
    private void SetStatusControl()
    {
      ...
      {
        btnAddForWO.Visible = true;
      }
      else
      {
        btnAddForWO.Visible = false;
      }
    ...
    private bool CheckData(out String message)
    {
      ...
      for (int i = 0; i < ultData.Rows.Count; i++)
      {
        ...
        // Check Purpose Of Requisition
        if (row.Cells["Remark"].Value.ToString().Trim().Length == 0)
        {
          message = "Purpose Of Requisition";
          return false;
        }

        // Project Code
        if (this.ultProjectCode.Value != null && this.ultProjectCode.Value.ToString().Length > 0
            && DBConvert.ParseInt(this.ultProjectCode.Value.ToString()) == int.MinValue)
        {
    ...
    private void ImportExcel()
    {
      ...
      try
      {
        ...
        if (this.flagHideWOCarcass)
        {
          dtSource = FunctionUtility.GetExcelToDataSet(txtImportExcel.Text.Trim(), "SELECT * FROM [Sheet1 (1)$B7:P1500]").Tables[0];
        }
        else
        {
          dtSource = FunctionUtility.GetExcelToDataSet(txtImportExcel.Text.Trim(), "SELECT * FROM [Sheet1 (1)$B7:I1500]").Tables[0];
        }
    ...
    private void ultData_InitializeLayout(object sender, InitializeLayoutEventArgs e)
    {
      ...
      e.Layout.Bands[0].Columns["Revision"].Hidden = this.flagHideWOCarcass ? false : true;
      e.Layout.Bands[0].Columns["ItemQty"].Hidden = this.flagHideWOCarcass ? false : true;
      e.Layout.Bands[0].Columns["PackingDeadline"].Hidden = this.flagHideWOCarcass ? false : true;
      e.Layout.Bands[0].Columns["WORemark"].Hidden = this.flagHideWOCarcass ? false : true;

      e.Layout.Bands[0].ColHeaderLines = 3;
      e.Layout.Bands[0].Columns["RequestDate"].Header.Caption = "(*)Required \n Delivery \n Date";
      e.Layout.Bands[0].Columns["QtyRequest"].Header.Caption = "Qty \n From \n Requester";
      ...
      e.Layout.Bands[0].Columns["Urgent"].MaxWidth = 60;
      e.Layout.Bands[0].Columns["Urgent"].MinWidth = 60;
      e.Layout.Bands[0].Columns["ProjectCode"].MaxWidth = 120;
      e.Layout.Bands[0].Columns["ProjectCode"].MinWidth = 120;
      e.Layout.Bands[0].Columns["Drawing"].MaxWidth = 60;
      e.Layout.Bands[0].Columns["Drawing"].MinWidth = 60;
      e.Layout.Bands[0].Columns["Sample"].MaxWidth = 60;
      e.Layout.Bands[0].Columns["Sample"].MinWidth = 60;
      ...
      if (this.flagHideWOCarcass)
      {
        ...
        e.Layout.Bands[0].Columns["CarcassCode"].Hidden = false;
    ...
    private void ultData_AfterCellUpdate(object sender, CellEventArgs e)
    {
      ...
      switch (columnName)
      {
        ...
        case "MaterialCode":
          ...
          if (dtRequestDate != null && dtRequestDate.Rows.Count > 0)
          {
    ...
    private void btnGettemplate_Click(object sender, EventArgs e)
    {
      ...
      if (this.flagHideWOCarcass)
      {
        templateName = "RPT_PUR_21_002_001";
      }
      else
      {
        templateName = "RPT_PUR_21_002";
      }
      string sheetName = "Sheet1";
      string outFileName = "Template Import PR Online";
      string startupPath = System.Windows.Forms.Application.StartupPath;
      string pathOutputFile = string.Format(@"{0}\Report", startupPath);
      string pathTemplate = startupPath + @"\ExcelTemplate\Purchasing";
    ...
    {
      viewPUR_21_009 uc = new viewPUR_21_009();
      WindowUtinity.ShowView(uc, "ADD DETAIL PR ONLINE", true, ViewState.ModalWindow, FormWindowState.Maximized);
      ...
      for (int i = 0; i < dtDetail.Rows.Count; i++)
      {
        ...
        rowAdd["WO"] = DBConvert.ParseInt(row["WO"].ToString());
        rowAdd["CarcassCode"] = row["CarcassCode"].ToString();
        rowAdd["ItemCode"] = row["ItemCode"].ToString();
        rowAdd["Revision"] = DBConvert.ParseInt(row["Revision"].ToString());
        rowAdd["Urgent"] = DBConvert.ParseInt(row["Urgent"].ToString());
        rowAdd["RequestDate"] = DBConvert.ParseDateTime(row["RequestDate"].ToString(), ConstantClass.USER_COMPUTER_FORMAT_DATETIME);
        rowAdd["Remark"] = row["Remark"].ToString();
        rowAdd["MaterialCode"] = row["MaterialCode"].ToString();
        rowAdd["MaterialNameVn"] = row["NameVN"].ToString();
        rowAdd["MaterialNameEn"] = row["NameEN"].ToString();
        rowAdd["Unit"] = row["Unit"].ToString();
        rowAdd["Qty"] = DBConvert.ParseDouble(row["Qty"].ToString());
    ...
    {
      if (this.ultProjectCode.Value != null && DBConvert.ParseInt(this.ultProjectCode.Value) != int.MinValue)
      {
        for (int i = 0; i < this.ultData.Rows.Count; i++)
        {
          UltraGridRow row = this.ultData.Rows[i];
          row.Cells["ProjectCode"].Value = DBConvert.ParseInt(this.ultProjectCode.Value);
        }
      }
    }
