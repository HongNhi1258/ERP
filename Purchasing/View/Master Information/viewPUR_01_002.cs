...
namespace DaiCo.Purchasing
{
  public partial class viewPUR_01_002 : MainUserControl
  {
    ...
    private void LoadData(string group)
    {
      ...
      string commandText = string.Format("SELECT Category, Name, Remark, LeadTimeDeadLinePacking, WarningPricePercent, FormulaStandardCostPercent, ISNULL(PurControl, 0) [Control], 0 AS Selected  FROM TblGNRMaterialCategory WHERE [Group] = {0}", value);
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
      ...
      if (checkSave)
      {
        foreach (DataRow row in dtSource.Rows)
        {
          ...
          //input[8] = new DBParameter("@UnitPrice", DbType.Double, price);
          DBParameter[] output = new DBParameter[] { new DBParameter("@Result", DbType.Int32, 0) };
          if (row.RowState == DataRowState.Added)
          {
