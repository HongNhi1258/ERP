/*
  Author      : 
  Date        : 29/06/2012
  Description : Mapping PRPO(Veneer)
*/
...
{
  public partial class viewPUR_07_006 : MainUserControl
  {
    #region Field
    // Pid Receiving
    ...
    // Store
    string sp_PURGetPORemainOfReceivingSpecial = "spPURGetPORemainOfReceivingSpecialVeneer";
    string sp_PURReceivingInfoSpecialByReveivingPid_Select = "spPURReceivingInfoSpecialVeneer_Select";
    string sp_PURReceivingSpecialMapingPOMaterials_Edit = "spPURReceivingSpecialMapingPOVeneer_Edit";
    string sp_PURReceivingSpecialStatusPRPOMaterials_Update = "spPURReceivingSpecialStatusPRPOVeneer_Update";
    string sp_PURReceivingSummaryDetailWarehouse_Update = "spPURReceivingSummaryDetailVeneer_Update";
    string sp_PURReceivingSummaryDetailWarehouse_Insert = "spPURReceivingSummaryDetailVeneer_Insert";
    #endregion Field

    #region Init
    public viewPUR_07_006()
    {
      InitializeComponent();
    }
    ...
    /// <param name="e"></param>
    private void viewPUR_07_006_Load(object sender, EventArgs e)
    {
    ...
    private bool CheckValid(out string message)
    {
      ...
      for (int p = 0; p < ultPurData.Rows.Count; p++)
      {
        UltraGridRow row1 = ultPurData.Rows[p];
        // Check Qty 
        if (row1.Cells["Qty"].Value.ToString().Length > 0)
        {
          if (DBConvert.ParseDouble(row1.Cells["Qty"].Value.ToString()) <= 0)
          {
            message = "Qty";
            return false;
          }
    ...
    private bool SaveUpdatePRPO()
    {
      ...
      // ReceivingPid
      inputParam[0] = new DBParameter("@IDPhieuNhap", DbType.Int64, this.receivingPid);
    ...
    private bool SaveReceivingVeneer()
    {
      for (int i = 0; i < ultPurData.Rows.Count; i++)
      {
        ...
        if (pid != long.MinValue)
        {
          ...
          // PO
          if (row.Cells["PO"].Value.ToString().Length > 0)
          {
            inputParamUpdate[1] = new DBParameter("@PONo", DbType.String, row.Cells["PO"].Value.ToString());
          }

          if (row.Cells["PR"].Value.ToString().Length > 0)
          {
            inputParamUpdate[2] = new DBParameter("@PRNo", DbType.String, row.Cells["PR"].Value.ToString());
          }
          // AlternativeCode
          if (row.Cells["AlternativeCode"].Value.ToString().Length > 0)
          {
            inputParamUpdate[3] = new DBParameter("@AlternativeCode", DbType.String, row.Cells["AlternativeCode"].Value.ToString());
          }
          // Qty
          if (DBConvert.ParseDouble(row.Cells["Qty"].Value.ToString()) != double.MinValue)
          {
            inputParamUpdate[4] = new DBParameter("@Qty", DbType.Double, DBConvert.ParseDouble(row.Cells["Qty"].Value.ToString()));
          }
          // UpdateBy
          inputParamUpdate[5] = new DBParameter("@UpdateBy", DbType.Int32, Shared.Utility.SharedObject.UserInfo.UserPid);

          // PODetailPid
          if (DBConvert.ParseLong(row.Cells["PODetailPid"].Value.ToString()) != long.MinValue)
          {
            inputParamUpdate[6] = new DBParameter("@PODetailPid", DbType.Int64, DBConvert.ParseLong(row.Cells["PODetailPid"].Value.ToString()));
          }
        ...
        {
          // Insert
          DBParameter[] inputParamInsert = new DBParameter[9];
          inputParamInsert[0] = new DBParameter("@ReceivingPid", DbType.String, this.receivingPid);
          ...
          if (row.Cells["PO"].Value.ToString().Length > 0)
          {
            inputParamInsert[3] = new DBParameter("@PONo", DbType.String, row.Cells["PO"].Value.ToString());
          }
          if (row.Cells["PR"].Value.ToString().Length > 0)
          {
            inputParamInsert[4] = new DBParameter("@PRNo", DbType.String, row.Cells["PR"].Value.ToString());
          }
          // AlternativeCode
          if (row.Cells["AlternativeCode"].Value.ToString().Length > 0)
          {
            inputParamInsert[5] = new DBParameter("@AlternativeCode", DbType.String, row.Cells["AlternativeCode"].Value.ToString());
          }
          // Qty
          if (DBConvert.ParseDouble(row.Cells["Qty"].Value.ToString()) != double.MinValue)
          {
            inputParamInsert[6] = new DBParameter("@Qty", DbType.Double, DBConvert.ParseDouble(row.Cells["Qty"].Value.ToString()));
          }
          // UpdateBy
          inputParamInsert[7] = new DBParameter("@UpdateBy", DbType.Int32, Shared.Utility.SharedObject.UserInfo.UserPid);
          // PODetailPid
          if (DBConvert.ParseLong(row.Cells["PODetailPid"].Value.ToString()) != long.MinValue)
          {
            inputParamInsert[8] = new DBParameter("@PODetailPid", DbType.Int64, DBConvert.ParseLong(row.Cells["PODetailPid"].Value.ToString()));
          }
