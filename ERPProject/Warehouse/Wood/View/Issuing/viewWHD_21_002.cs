/*
  Author      : Duong Minh
  Date        : 14/06/2012
  Description : Insert/Update Issuing For Warehouse From Supplier (PR-PO)
*/
...
{
  public partial class viewWHD_21_002 : MainUserControl
  {
    #region Field
    // Pid Issuing
    ...
    private void LoadData()
    {
      ...
      if (dtReceivingInfo.Rows.Count > 0)
      {
        ...
        this.txtAutoSearch.Text = row["RecevingPerson"].ToString();
        this.ultDepartment.Value = row["IssuingSource"].ToString();
        this.ultReceiver.Value = DBConvert.ParseInt(row["RecevingPerson"].ToString());
      ...
      {
        DataTable dtReceiving = DataBaseAccess.SearchCommandTextDataTable("SELECT dbo.FWHDGetNewIssuingNoWoods('05ISS') NewIssuingNo");
        if ((dtReceiving != null) && (dtReceiving.Rows.Count > 0))
        {
          ...
          //ultReceipt.DataSource = uc.dtDetail;
        }
      }
    ...
    private void SetStatusControl()
    {
      ...
      this.ultDepartment.Enabled = this.canUpdate;
      this.txtAutoSearch.Enabled = this.canUpdate;
      this.ultReceiver.Enabled = this.canUpdate;
    ...
    private bool CheckValid(out string message)
    {
      ...
      {
        string commandText = string.Empty;
        commandText = "SELECT COUNT(*) FROM VHRDDepartment WHERE Department = '" + this.ultDepartment.Value.ToString() + "'";
      ...
      // Receiver
      if (this.ultReceiver.Value != null && this.ultReceiver.Value.ToString().Length > 0)
      {
        string commandText = string.Empty;
        commandText = "SELECT COUNT(*) FROM VHRNhanVien WHERE ID_NhanVien = " + DBConvert.ParseInt(this.ultReceiver.Value.ToString());
        DataTable dtCheck = DataBaseAccess.SearchCommandTextDataTable(commandText);
        if (dtCheck != null && dtCheck.Rows.Count > 0)
        {
          if (DBConvert.ParseInt(dtCheck.Rows[0][0].ToString()) == 0)
    ...
    private bool SaveIssuingVeneer()
    {
      ...
      if (this.issuingPid != long.MinValue)
      {
        ...
        // Issuing Source
        inputParamUpdate[3] = new DBParameter("@IssuingSource", DbType.String, this.ultDepartment.Value.ToString());

        // Receiving Person
        inputParamUpdate[4] = new DBParameter("@RecevingPerson", DbType.Int32, DBConvert.ParseInt(this.ultReceiver.Value.ToString()));

        // Type
        inputParamUpdate[5] = new DBParameter("@Type", DbType.Int32, 1);

        // Remark
        inputParamUpdate[6] = new DBParameter("@Remark", DbType.AnsiString, 4000, this.txtRemark.Text);
      ...
      else
      {
        ...
        // Import Source
        inputParamInsert[2] = new DBParameter("@IssuingSource", DbType.String, this.ultDepartment.Value.ToString());

        // Receving Person
        inputParamInsert[3] = new DBParameter("@RecevingPerson", DbType.Int32, DBConvert.ParseInt(this.ultReceiver.Value.ToString()));

        // Type
        inputParamInsert[4] = new DBParameter("@Type", DbType.Int32, 1);

        // Remark
        inputParamInsert[5] = new DBParameter("@Remark", DbType.AnsiString, 4000, this.txtRemark.Text);

        // CreateBy
        inputParamInsert[6] = new DBParameter("@CreateBy", DbType.Int32, SharedObject.UserInfo.UserPid);

        // ReceivingNo
        inputParamInsert[7] = new DBParameter("@IssuingNo", DbType.String, "05ISS");
