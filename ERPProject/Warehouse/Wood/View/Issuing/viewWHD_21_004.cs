/*
  Author      : Duong Minh
  Date        : 14/6/2012
  Description : Insert/Update Adjustment Out
*/
...
{
  public partial class viewWHD_21_004 : MainUserControl
  {
    #region Field
    // Pid Issuing
    ...
    private void LoadData()
    {
      ...
      if (dtReceivingInfo.Rows.Count > 0)
      {
        ...
        this.ultApprovedBy.Value = row["ApprovedPerson"].ToString();
      ...
      {
        DataTable dtReceiving = DataBaseAccess.SearchCommandTextDataTable("SELECT dbo.FWHDGetNewIssuingNoWoods('05ADO') NewIssuingNo");
    ...
    private void ultReceipt_AfterCellUpdate(object sender, CellEventArgs e)
    {
      ...
      switch (columnName)
      {
        case "LotNoId":
          ...
          commandText += " 		0 PID, SB.LotNoId LotNoId, SB.MaterialCode, MAT.TenEnglish NameEN,";
          commandText += " 		MAT.TenVietNam NameVN, UNI.TenDonViEN, ISNULL(SB.Qty, 0) Qty, ISNULL(DIM.[Length], 0) [Length],";
          commandText += " 		ISNULL(DIM.Width, 0) Width, ISNULL(DIM.Thickness, 0) Thickness, ";
          commandText += " 		(ISNULL(SB.Qty, 0) * ISNULL(DIM.[Length], 0) * ISNULL(DIM.Width, 0) * ISNULL(DIM.Thickness, 0)/1000000000) TotalCBM,";
          commandText += " 		ISNULL(DIMEXI.[Length], 0) [LengthEXI], ISNULL(DIMEXI.Width, 0) WidthEXI, ISNULL(DIMEXI.Thickness, 0) ThicknessEXI, ";
          commandText += " 		(ISNULL(SB.Qty, 0) * ISNULL(DIMEXI.[Length], 0) * ISNULL(DIMEXI.Width, 0) * ISNULL(DIMEXI.Thickness, 0)/1000000000) TotalCBMEXI,";
          commandText += " 		LOC.Name Location, ISNULL(SB.Qty, 0) IssueQty";
          commandText += " FROM TblWHDStockBalance SB ";
          commandText += "    LEFT JOIN VWHDMaterialCodeCommon MAT ON MAT.ID_SanPham = SB.MaterialCode";
          commandText += "    LEFT JOIN VWHDUnit UNI ON MAT.IDDonViTinh = UNI.ID_DonViTinh";
          commandText += "    LEFT JOIN VWHDDimensionWoods DIM ON DIM.Pid = SB.DimensionPid";
          commandText += "    LEFT JOIN VWHDDimensionWoods DIMEXI ON DIMEXI.Pid = SB.DimensionPidEXI";
          commandText += "    LEFT JOIN VWHDLocationWoods LOC ON SB.LocationPid = LOC.ID_Position";
          commandText += " WHERE SB.LotNoId ='" + lotnoId + "'";
    ...
    private bool SaveIssuingVeneer()
    {
      ...
      else
      {
        ...
        // ReceivingNo
        inputParamInsert[6] = new DBParameter("@IssuingNo", DbType.String, "05ADO");
