...
{
  public partial class viewWHD_20_003 : MainUserControl
  {
    #region Field
    // Pid Receiving
    ...
    /// <param name="e"></param>
    private void viewWHD_20_003_Load(object sender, EventArgs e)
    {
      ultCBCustomerDate.Value = DBNull.Value;      
    ...
    private void LoadComboSupplier()
    {
      ...
      commandText = "SELECT ID_NhaCC, TenNhaCCEN + ' - ' + ID_NhaCC AS Name FROM VWHDSupplier";
      if (this.receivingPid == long.MinValue || this.status == 0)
      {
        commandText += " WHERE IDTinhTrang IN (1, 2)";
      }
      commandText += " ORDER BY TenNhaCCEN";
      System.Data.DataTable dtSource = DataBaseAccess.SearchCommandTextDataTable(commandText);

      ultSupplier.DataSource = dtSource;
      ultSupplier.DisplayMember = "Name";
      ultSupplier.ValueMember = "ID_NhaCC";
      ultSupplier.DisplayLayout.Bands[0].ColHeadersVisible = true;
    ...
    private void LoadData()
    {
      ...
      if (dtReceivingInfo.Rows.Count > 0)
      {
        ...
        this.txtInvoiceNote.Text = row["InvoiceNote"].ToString();
        this.txtCustomerNo.Text = row["CustomNo"].ToString();
        this.ultCBCustomerDate.Value = row["CustomDate"].ToString();
        if (this.status > 0)
        {
      ...
      {
        DataTable dtReceiving = DataBaseAccess.SearchCommandTextDataTable("SELECT dbo.FWHDGetNewReceivingNoWoods('05REC') NewReceivingNo");
    ...
    private void SetStatusControl()
    {
      ...
      this.txtInvoiceNote.Enabled = this.canUpdate;
      this.txtCustomerNo.Enabled = this.canUpdate;
      this.ultCBCustomerDate.Enabled = this.canUpdate;
    ...
    private bool SaveReceivingWoods()
    {
      ...
      if (this.receivingPid != long.MinValue)
      {
        ...
        DBParameter[] inputParamUpdate = new DBParameter[10];
        ...
        // Type
        inputParamUpdate[4] = new DBParameter("@Type", DbType.Int32, 1);
        ...
        // Invoice Note
        if (txtInvoiceNote.Text.Length > 0)
        {
          inputParamUpdate[7] = new DBParameter("@InvoiceNote", DbType.String, txtInvoiceNote.Text);
        }
        // Customer No
        if (txtCustomerNo.Text.Length > 0)
        {
          inputParamUpdate[8] = new DBParameter("@CustomNo", DbType.String, txtCustomerNo.Text);
        }
        // customer Date
        if (ultCBCustomerDate.Value != null)
        {
          inputParamUpdate[9] = new DBParameter("@CustomDate", DbType.DateTime, (DateTime)ultCBCustomerDate.Value);
        }
      ...
      else
      {
        ...
        DBParameter[] inputParamInsert = new DBParameter[10];
        ...
        // Type
        inputParamInsert[3] = new DBParameter("@Type", DbType.Int32, 1);
        ...
        // ReceivingNo
        inputParamInsert[6] = new DBParameter("@ReceivingNo", DbType.String, "05REC");

        // Invoice Note
        if (txtInvoiceNote.Text.Length > 0)
        {
          inputParamInsert[7] = new DBParameter("@InvoiceNote", DbType.String, txtInvoiceNote.Text);
        }
        // Customer No
        if (txtCustomerNo.Text.Length > 0)
        {
          inputParamInsert[8] = new DBParameter("@CustomNo", DbType.String, txtCustomerNo.Text);
        }
        // customer Date
        if (ultCBCustomerDate.Value != null)
        {
          inputParamInsert[9] = new DBParameter("@CustomDate", DbType.DateTime, (DateTime)ultCBCustomerDate.Value);
        }
    ...
    private bool CheckValid(out string message)
    {
      ...
      for (int i = 0; i < this.ultReceipt.Rows.Count; i++)
      {
        ...
        string material = DBConvert.ParseString(rowGrid.Cells["MaterialCode"].Value.ToString());
        ...
        {
          string ccommand = "SELECT MaterialCode FROM TblGNRMaterialInformation WHERE Source = 2 AND MaterialCode = '" + material + "' ";
          DataTable dt = DataBaseAccess.SearchCommandTextDataTable(ccommand);
          if (dt != null && dt.Rows.Count > 0)
          {
            if (txtCustomerNo.Text.Length == null || ultCBCustomerDate.Value == null)
            {
              message = "Custom";
              return false;
            }
        ...
        //DataTable dt1 = DataBaseAccess.SearchStoreProcedureDataTable("spWHDCheckValidRecevingWood");
        //if (dt1.Rows[0]["Display"].ToString() == "1")
        //{
