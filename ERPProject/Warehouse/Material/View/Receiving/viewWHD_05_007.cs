/*
  ...
  Date        : 22/12/2012
  Description : Receivng From Supplier(Not PO)
*/
...
{
  public partial class viewWHD_05_007 : MainUserControl
  {
    #region Field
    private int whPid = 0;
    //RTW
    ...
    #region Init
    public viewWHD_05_007()
    {
      InitializeComponent();
    }

    private void viewWHD_05_007_Load(object sender, EventArgs e)
    {
    ...
    {
      DBParameter[] inputParam = new DBParameter[] { new DBParameter("@RTWNo", DbType.AnsiString, 50, this.receivingNo) };
      ...
      if (dtRTWInfo.Rows.Count > 0)
      {
        ...
        ultCBSupplier.Value = dtRTWInfo.Rows[0]["Department"].ToString();
        txtInvoiceNote.Text = dtRTWInfo.Rows[0]["Reason"].ToString();
      ...
      {
        DataTable dtRTW = DataBaseAccess.SearchCommandTextDataTable("SELECT dbo.FWHDGetNewReceivingNoForMaterial('03REC') NewRTWCode");
    ...
    {
      if (this.receivingNo == string.Empty)
      {
        ...
        if (ultCBSupplier.Text.Trim().Length > 0 && ultCBSupplier.Value != null)
        {
          inputParam[6] = new DBParameter("@Supplier", DbType.String, 50, ultCBSupplier.Value.ToString());
        }
        if (txtInvoiceNote.Text.Length > 0)
        {
          inputParam[7] = new DBParameter("@SupplierNote", DbType.String, 255, txtInvoiceNote.Text);
        }
        ...
        DataBaseAccess.ExecuteStoreProcedure("spWHDReceivingInfomation_Insert", inputParam, outputParam);
      ...
      else
      {
        ...
        if (ultCBSupplier.Text.Trim().Length > 0 && ultCBSupplier.Value != null)
        {
          inputParam[4] = new DBParameter("@Supplier", DbType.String, 50, ultCBSupplier.Value.ToString());
        }
        if (txtInvoiceNote.Text.Length > 0)
        {
          inputParam[5] = new DBParameter("@SupplierNote", DbType.String, txtInvoiceNote.Text);
        }
        ...
        DataBaseAccess.ExecuteStoreProcedure("spWHDReceivingInfomation_Update", inputParam, outputParam);
      ...
      if (ultData.Rows.Count > 0)
      {
        for (int i = 0; i < ultData.Rows.Count; i++)
        {
          ...
          if (DBConvert.ParseInt(rowInfo.Cells["Flag"].Value.ToString()) == 1)
          {
            ...
            inputParam[1] = new DBParameter("@RTWNo", DbType.AnsiString, 50, this.receivingNo);
    ...
    private bool UpdateStockBalance()
    {
      ...
      {
        DBParameter[] input = new DBParameter[1];
        input[0] = new DBParameter("@IDPhieuNhap", DbType.AnsiString, 50, this.receivingNo);
        DBParameter[] output = new DBParameter[1];
        output[0] = new DBParameter("@Result", DbType.Int64, long.MinValue);
    ...
    {
      DBParameter[] input = new DBParameter[1];
      input[0] = new DBParameter("@RTWNo", DbType.AnsiString, 50, this.receivingNo);
      DBParameter[] output = new DBParameter[1];
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
      if (ultData.Rows.Count == 0)
      {
        WindowUtinity.ShowMessageError("ERR0001", "Data Row");
        return;
      }
