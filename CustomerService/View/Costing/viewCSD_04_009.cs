...
{
  public partial class viewCSD_04_009 : MainUserControl
  {
    #region Fields
    ...
    /// </summary>
    private void viewCSD_04_009_Load(object sender, EventArgs e)
    {
    ...
    private void ChkAll_CheckedChange(int rowIndex) {
      string prefix = (rowIndex == 0) ? "US" : (rowIndex == 1) ? "DC" : (rowIndex == 2) ? "UK" : "DCI";
    ...
    private void LoadData()
    {
      ...
      DataTable dataSource = DataBaseAccess.SearchStoreProcedureDataTable("spCSDItemSalePrice_SelectByFactory", 36000, input, output);
    ...
    private void ultShowColumn_CellChange(object sender, CellEventArgs e)
    {
      string columnName = e.Cell.Column.ToString().Trim();
      UltraGridRow row = e.Cell.Row;
      string prefix = (row.Index == 0) ? "US" : (row.Index == 1) ? "DC" : (row.Index == 2) ? "UK" : "DCI";
